...
// $user->setUsername($conn, 'aferling');
...
$generateMembersSQL = "SELECT DISTINCT users.userID, username, name, section FROM users INNER JOIN member_groups ON users.userID = member_groups.userID WHERE section IN ($sqlSections) AND groupID IN ($eventGroups) ORDER BY section, name, username";
...
<div class="container">
    ...
    <div class="card-panel">
        ...
        <ul class="collection" id="stations">
            ...
            $currentSection = $userLoop['section']; ?>
            <!-- todo add an autfilling write in for other section members -->
        </ul>
    </div>
    <div class="card-panel">
        <h5 class="card-title"><?php echo $currentSection; ?></h5>
    ...
    <div class='row'>
        <a class='submit-attendance waves-effect waves-light btn green darken-2 col s12 m12 l12'>submit attempt</a>
    </div>
</div>
